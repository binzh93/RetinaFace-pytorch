...
#         print("---------------------------")
#         a_r = []
#         for kkk in range(len(targets[0])):
#             a_r.append(targets[0][kkk].shape)
#         print("boxes shape: ", a_r)
        ...
        for idx in range(batch):
            ...
            print(anchors_cp.shape)
            print(gt_boxes.shape)
            t_overlap = time.time()
            overlaps = box_overlaps(anchors_cp, gt_boxes)  # support box 5 point
            t_overlap2 = time.time()
            ovlap_time_comp += (t_overlap2 - t_overlap)
...
#         print(bbox_targets_t.device)
#         print(bbox_targets_t[0].device)
...
#         valid_idx = anchors_label_t>=0   # (batch, 16800)
...
#             conf_t = anchors_label_t[:, v_idx].view(-1,)
            conf_t = anchors_label_t[:, inds[i]: inds[i+1]].contiguous().view(-1,)
            
#             loc_pred = loc_pred_batch[:, v_idx, :].view(-1, bbox_pred_len)
#             import pdb
#             pdb.set_trace()
            pos = anchors_label_t[:, inds[i]: inds[i+1]] > 0
            pos_loc_idx = pos.unsqueeze(pos.dim()).expand_as(loc_pred_batch[i])
            loc_pred = loc_pred_batch[i][pos_loc_idx].view(-1, bbox_pred_len)
#             loc_t = bbox_targets_t[:, v_idx, :].view(-1, bbox_pred_len)
#             pos_loc_idx = valid_idx.unsqueeze(valid_idx.dim()).expand_as(bbox_targets_t)
            loc_t = bbox_targets_t[:, inds[i]: inds[i+1], :][pos_loc_idx].view(-1, bbox_pred_len)
            bbox_weights_temp = bbox_weights_t[:, inds[i]: inds[i+1], :][pos_loc_idx].view(-1, bbox_pred_len)
...
#                 landmark_pred = landmark_targets_t[:, v_idx, :].view(-1, landmark_pred_len)
                pos_landmark_idx = pos.unsqueeze(pos.dim()).expand_as(landmark_pred_batch[i])
    
                landmark_pred = landmark_pred_batch[i][pos_landmark_idx].view(-1, landmark_pred_len)
                landmark_t = landmark_targets_t[:, inds[i]: inds[i+1], :][pos_landmark_idx].view(-1, landmark_pred_len)
                landmark_weight_temp = landmark_weights_t[:, inds[i]: inds[i+1], :][pos_landmark_idx].view(-1, landmark_pred_len)
...
#             print("N_landmark: ", N_landmark)
        ...
        print("SSSSSSSSSSSS")
        print("overlap time: ", ovlap_time_comp)
        print("other: ", t2 -t1)
